...
from apps.stems.services.stem_service import request_stem_separation, FileTooLargeError, InsufficientCreditsError
...
class StemJobStatusView(APIView):
    ...
    def get(self, request, id):
        try:
            ...
            stem_files = [
                {'stem_type': sf.stem_type, 'id': str(sf.id)}
                for sf in job.stem_files.all()
            ]
            return Response({
                'status': job.status,
...
class StemFileDownloadView(APIView):
    ...
    def get(self, request, id):
        try:
            ...
            stem_file = StemFile.objects.select_related('stem_job').get(id=id, stem_job__user=request.user)
